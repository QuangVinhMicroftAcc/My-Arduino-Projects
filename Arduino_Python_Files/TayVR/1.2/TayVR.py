...
try:
    ...
    tkinter.messagebox.showerror("ERROR: Cannot decode ASCII with the given inputs","We're not sure that are you so unluck that you can even encountered this error. You can restart the driver.")
    run = False
    exit()
try:
...
while run:
    try:
        ...
        tkinter.messagebox.showerror("ERROR: Cannot decode ASCII with the given inputs","We're not sure that are you so unlucky that you can even encountered this error. You can restart the driver but please refrain from touching the cables to much. They are case sensitive.")
        run = False
        exit()
        ...
        tkinter.messagebox.showerror("ERROR: Serial Exception","Looks like your device is not sending any states or movement infos, please check your cable if it's plugged in correctly, and try again. If the issue persists, call someone have knowledge on computers, specifically python and Arduino boards. And if you don't know anyone, you can accept: YOU LOST")
        run = False
        exit()
    except:
        if checkclose == False:
