...
tkinter.messagebox.showinfo('Arduino Controller Driver 1.1','To disable the controller, please press Ctrl+Alt+M. Using controller will result in PERMANENT DISABLE of the Windows Key')
while run:
    # if mode == "2d":
    # up = 'up arrow'
    # down = 'down arrow'
    ...
    (x,y) = (x-xt, y-yt)
    (x,y) = (x/40,y/40)
    if x >= 600 or x <= 400 or y >= 600 or y <= 400:
        for i in range(4):
            (Xo,Yo) = mouse.get_position()
